...
- **Via Mise**
  ...
  ```bash
  mise use -g github:liangboceo/opencode
  ```

- **Via Docker**

  ```bash
  docker run -it --rm ghcr.io/liangboceo/opencode
  ```

Le support de l'installation de OpenCode sur Windows à l'aide de Bun est actuellement en cours de développement.

Vous pouvez également récupérer le binaire dans les [Releases](https://github.com/liangboceo/opencode/releases).
